...
women_mental_health__more_than_10_days <- subset(women_diabetes_dataset, 
                                                women_diabetes_dataset$MentHlth >= 12)

women_mental_health__more_than_10_days <- subset(women_diabetes_dataset, 
                                                women_diabetes_dataset$MentHlth >= 1)
...
Vamos a crear 
```{r}
sqldf("SELECT Diabetes_012, HighBP, HighChol, AVG(Income), AVG(Age), 
        Count(*) * 100.0/ SUM(COUNT(*)) OVER () as percentage
      FROM women_diabetes_dataset
      GROUP BY Diabetes_012, HighBP, HighChol")
```
```{r}
sqldf("SELECT Diabetes_012, HighBP, HighChol, AVG(Income), AVG(Age),                           Count(*) * 100.0 / SUM(COUNT(*)) OVER () as percentage
      FROM women_mental_health__more_than_3_days
      GROUP BY Diabetes_012, HighBP, HighChol")
```
Hemos encontrado conclusiones muy interesantes:

 * Las mujeres con más problemas mentales, tienden 
```{r}
sqldf("SELECT Diabetes_012, HighBP, HighChol, AVG(HvyAlcoholConsump), AVG(Income),             AVG(Age), Count(*) * 100.0/SUM(COUNT(*)) OVER () as percentage
