...
```{r}
summary_all_women <- sqldf(diabetes_bp_chol_summary("all_women"))
summary_all_women

summary_women_stable_mental_health <- sqldf(diabetes_bp_chol_summary("women_stable_mental_health"))
...
```
De media, las mujeres con más inestabilidad emocional con respecto a las más estables tienden a:
...
```{r}
lm_all_women <- lm(BMI ~ Income, 
                   data = all_women)
lm_women_stable_mental_health <- lm(BMI ~ Income, 
                                    data = women_stable_mental_health)

lm_women_mental_health_problems_more_than_10_days_a_month <- lm(
  BMI ~ Income, 
  data = women_mental_health_problems_more_than_10_days_a_month)

ggplot(all_women, 
       aes(x = Income, y = BMI, color = dataset)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, 
              formula = lm_all_women) +
  geom_smooth(method = "lm", se = FALSE, 
              formula = lm_women_stable_mental_health, 
              linetype = "dashed") +
  geom_smooth(method = "lm", se = FALSE, 
              formula = lm_women_mental_health_problems_more_than_10_days_a_month, 
              linetype = "dotted") +
  scale_color_manual(values = c("Dataset 1" = "blue", "Dataset 2" = "green", "Dataset 3" = "red"))
```
